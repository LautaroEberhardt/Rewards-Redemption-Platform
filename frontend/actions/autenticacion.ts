...
export async function registrarUsuario(datos: FormularioRegistroDatos) {
  ...
  try {
    ...
    if (!respuestaBackend.ok) {
      ...
      return {
        error: errorData.message || "Error al crear el usuario en el sistema.",
      };
    }
  } catch (error) {
  ...
  try {
    ...
    if (error instanceof AuthError) {
      return {
        error:
          "Cuenta creada exitosamente, pero falló el inicio de sesión automático. Por favor ingresa manualmente.",
      };
    }
